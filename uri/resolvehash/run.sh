...
WT="$PROJ/work/WT"
mkdir -p "$WT"
( cd "$WT" && "$BE" get "file:$PROJ/.be?/alpha" ) >/dev/null 2>&1 || _fail "could not clone the worktree"
...
SUB="$WT/sub"
mkdir -p "$SUB"
( cd "$SUB" && "$BE" get "file:$PROJ/.be?/alpha" ) >/dev/null 2>&1 || _fail "could not clone the sub"
