...
# SRC_ROOT holds `parent` (the //parent tree) with a nested `sub` worktree.
export SRC_ROOT="$WORK"

# parent: a colocated primary store (pre-made `.be` shield, then a commit).
mkdir -p "$WORK/parent/.be"
( cd "$WORK/parent" && printf 'PARENT-A\n' > a.txt \
...
mkdir -p "$WORK/parent/sub"
( cd "$WORK/parent/sub" && "$JABC" get "file:$WORK/parent/.be?/" >/dev/null 2>&1 ) \
    || _fail "sub secondary clone failed"
[ -f "$WORK/parent/sub/.be" ] || _fail "sub/.be is not a FILE redirect (setup broke)"

# Snapshot the SUB's `.be` and the PARENT's wtlog before the nav get.
cp "$WORK/parent/sub/.be" "$WORK/sub.before"
par_before=$(wc -l < "$WORK/parent/.be/wtlog")

# THE ACT: a bare `:get` under the `//parent` nav context, cwd = the sub.
if ( cd "$WORK/parent/sub" && "$JABC" get '//parent' ) >"$WORK/get.out" 2>&1; then
    :
else
...
if ! cmp -s "$WORK/sub.before" "$WORK/parent/sub/.be"; then
...
par_after=$(wc -l < "$WORK/parent/.be/wtlog")
