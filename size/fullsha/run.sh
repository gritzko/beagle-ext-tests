...
[ -n "$JABC" ] && [ -x "$JABC" ] || { echo "size: cannot locate jab (set BIN=)" >&2; exit 2; }
_BIN=$(dirname "$JABC")
BE=$JABC
...
parity() {
    ...
    #  URI-014: drop the hunk word-URI banner `size <uri>` (embeds the query sha —
    #  differs full vs short) so the BODY (the size) compares.
    sed -E '/^size /d' "$WORK/short.$_lbl" >"$WORK/short.$_lbl.n"
    sed -E '/^size /d' "$WORK/full.$_lbl"  >"$WORK/full.$_lbl.n"
